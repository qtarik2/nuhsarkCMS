...
$con,
"SELECT COUNT(*) As total_records FROM madrasati_expenses WHERE Date LIKE '%$Date' ORDER BY ID ASC"
);
...
$count=1;
$sel_query="SELECT * FROM madrasati_expenses WHERE Date LIKE '%$Date' ORDER BY ID ASC LIMIT $offset, $total_records_per_page;";
$result = mysqli_query($con,$sel_query);
while($row = mysqli_fetch_assoc($result)) { ?>
<tr>
...
$Status=$_REQUEST["Status"];
	...
	$update="UPDATE madrasati_expenses SET Date='".$Date."', Name='".$Name."', Amount='".$Amount."', AccountNo='".$AccountNo."', AdditionalInfo='".$AdditionalInfo."', Status='".$Status."'  WHERE ID='".$ID."'";
...
// Calculation
    	...
    	$QuerySum1 = "SELECT SUM(Amount) FROM madrasati_expenses WHERE Date LIKE '%$Date'";
	 	...
	 	$QuerySum2 = "SELECT COUNT(Date) FROM madrasati_expenses WHERE Date LIKE '%$Date'";
...
BULK UPDATE
<p><button type="submit" class="btn btn-default">[UNDER DEVELOPMENT] BULK UPDATE</button></p>
