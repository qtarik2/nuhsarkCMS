...
$count=1;
$sel_query="SELECT * FROM nuhsark_accountsummary WHERE StudentID LIKE '$Month%'";
// $sel_query="SELECT * FROM nuhsark_accountsummary WHERE Year LIKE '%$Month' ORDER BY StudentID ASC;";
...
// count($id_array) --> if I input 4 fields, count($id_array) = 4)
   ...
   $StudentID[] = mysql_real_escape_string($StudentID_array[$i]);
   $M01[] = mysql_real_escape_string($M01_array[$i]);
   $M02[] = mysql_real_escape_string($M02_array[$i]);
   $M03[] = mysql_real_escape_string($M03_array[$i]);  
   $M04[] = mysql_real_escape_string($M04_array[$i]);
   $M05[] = mysql_real_escape_string($M05_array[$i]);
   $M06[] = mysql_real_escape_string($M06_array[$i]);
   $M07[] = mysql_real_escape_string($M07_array[$i]);
   $M08[] = mysql_real_escape_string($M08_array[$i]);
   $M09[] = mysql_real_escape_string($M09_array[$i]);
   $M10[] = mysql_real_escape_string($M10_array[$i]);
   $M11[] = mysql_real_escape_string($M11_array[$i]);
   $M12[] = mysql_real_escape_string($M12_array[$i]);                  
}

	$update="UPDATE nuhsark_accountsummary SET M01='".$M01."', M02='".$M02."', M03='".$M03."', M04='".$M04."', M05='".$M05."', M06='".$M06."', M07='".$M07."', M08='".$M08."', M09='".$M09."', M10='".$M10."', M11='".$M11."', M12='".$M12."' WHERE StudentID='".$StudentID."'";
	mysqli_query($con, $update) or die(mysqli_error());
	
// End Of Update Algorithm
