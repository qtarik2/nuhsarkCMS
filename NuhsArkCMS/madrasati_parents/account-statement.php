...
$con,
"SELECT COUNT(*) As total_records FROM madrasati_accountsummary WHERE StudentID='$Username'"
);
...
// $Username=$_SESSION['username'];
$count=1;
$sel_query="SELECT * FROM madrasati_accountsummary WHERE StudentID='$Username' LIMIT $offset, $total_records_per_page;";
...
// Calculation
    	$QuerySum1 = "SELECT SUM(Amount) FROM madrasati_invoice WHERE StudentID='$Username'";
	 	...
	 	$QuerySum2 = "SELECT SUM(Amount) FROM madrasati_payment WHERE StudentID='$Username'";
		...
		$updateMYR = "UPDATE madrasati_accountsummary SET Debit='$InvoicesMYR', Credit='$PaymentsMYR', Balance='$BalanceMYR' WHERE StudentID='$Username'";
...
// Calculation
    	$QuerySum1 = "SELECT SUM(Debit) FROM madrasati_accountsummary WHERE StudentID='$Username'";
	 	if ($result1 = mysqli_query($con, $QuerySum1)) {
		    /* fetch associative array */
		    while ($row = mysqli_fetch_array($result1)) {
    	...
    	$QuerySum2 = "SELECT SUM(Credit) FROM madrasati_accountsummary WHERE StudentID='$Username'";
	 	if ($result2 = mysqli_query($con, $QuerySum2)) {
		    /* fetch associative array */
		    while ($row = mysqli_fetch_array($result2)) {
